...
def test_parse_all_tool_subtask_plan_to_route_decision_uses_tool_route() -> None:
    planner = MainPlanner(provider=None, model=None, config=PlannerConfig(enabled=True))

    decision = planner.parse_decision(
        '{"route":"plan","confidence":0.88,"reason":"Public lookup",'
        '"subtasks":['
        '{"route":"web_search","tool":"web_search","task":"查询深圳天气"},'
        '{"route":"web_search","tool":"web_search","task":"查询深圳空气质量"}'
        "]}",
        original_task="查询深圳天气和空气质量",
    )
