    ...
    if profile == "qwen3vl":
        return {
            ...
            "rules": (
                ...
                "- If the task names a target app and the foreground app is wrong, make your first action the `open` action for that app. Example: `<tool_call>{\"name\":\"mobile_use\",\"arguments\":{\"action\":\"open\",\"text\":\"Taobao\"}}</tool_call>`. Do not use swipe gestures to hunt for apps or coordinate taps to navigate inside the wrong app.",
