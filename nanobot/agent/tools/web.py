...
class WebSearchTool(Tool):
    ...
    description = (
        ...
        "count defaults to configured max_results (usually 5). If snippets are enough to answer, do not call web_fetch."
    )
    ...
    async def execute(self, query: str, count: int | None = None, **kwargs: Any) -> str:
        provider = self.config.provider.strip().lower() or "brave"
        default_count = min(max(self.config.max_results, 1), 10)
...
class WebFetchTool(Tool):
    ...
    name = "web_fetch"
    description = (
        "Fetch a URL and extract readable content (HTML → markdown/text). Output is capped "
        "at maxChars (default 50000). Works for most web pages and docs; may fail on "
        "login-walled or JS-heavy sites."
    )
