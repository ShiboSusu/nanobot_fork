...
class CostAwareProblemRouter:
    ...
    def classify(
        ...
        *,
        ...
        if self._is_open_settings_task(normalized) and "gui_task" in available:
            return RouteDecision(
    ...
    def _is_open_settings_task(self, normalized: str) -> bool:
        return (
            any(term in normalized for term in self._OPEN_TERMS)
            and any(term in normalized for term in self._SETTINGS_TERMS)
        )
